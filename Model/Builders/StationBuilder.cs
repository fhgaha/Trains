    ...
    public class StationBuilder : Spatial
    {
        ...
        private void UpdateBlueprintForMm()
        {
            ...
            var closestCell = cells
                .OrderBy(c => Math.Abs(c.Row + Cell.Size - pos.x))
                .ThenBy(c => Math.Abs(c.Col + Cell.Size - pos.z))
                .First();

            blueprint.Translation = new Vector3(closestCell.Row + (float)Cell.Size/2, 0f, closestCell.Col + (float)Cell.Size/2);
