...
{
	[Tool]
	public class CellNoScene : Spatial
	{
		public static CellNoScene[,] Cells { get; set; }
		...
		public static float GetDistance(CellNoScene first, CellNoScene second)
		=> (float)Math.Sqrt(Math.Pow(first.Row - second.Row, 2) + Math.Pow(first.Col - second.Col, 2));

		public static float GetDistanceSquared(CellNoScene first, CellNoScene second)
		=> (float)(Math.Pow(first.Row - second.Row, 2) + Math.Pow(first.Col - second.Col, 2));

		internal void CargoArrived(Cargo cargo, ProductType productType, float amount, CellNoScene[,] cells)
		{
